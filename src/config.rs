...
pub const APP_NAME: &str = "mobius";
/// Local fallback config path.
pub const CONFIG_PATH: &str = "config/mobius.toml";
/// Label used for the terminal present texture (sampled by the materials).
pub const TERMINAL_TEXTURE_LABEL: &str = "mobius.parley_ratatui";
/// Label used for the terminal render target (Vello's storage texture).
pub const TERMINAL_RENDER_TEXTURE_LABEL: &str = "mobius.parley_ratatui.render";
...
impl AppConfig {
    ...
    fn default_config_path() -> anyhow::Result<Option<PathBuf>> {
        ...
        let system_path = strategy.config_dir().join(APP_NAME).join("mobius.toml");
...
"#,
    ...
    fn distributed_config_binds_every_camera_slot() {
        let config: AppConfig =
            toml::from_str(include_str!("../config/mobius.toml")).expect("distributed config");
