//! rio-vt integration.
//!
//! Holds the [`EventListener`] mobius drives rio-vt with, the screen-reading
...
//! That function deep-copies every visible row on each call, and mobius reads the
...
/// The rio-vt state machine mobius drives.
pub type VtTerminal = Crosswords<TerminalEventSink>;

/// Sink for the events rio-vt raises while parsing.
///
/// The only variant mobius acts on today is [`RioEvent::PtyWrite`] — the
...
impl EventListener for TerminalEventSink {
    fn send_event(&self, event: RioEvent, _window: WindowId) {
        match event {
            ...
            // invoke and write back. Mobius answers none of them yet, so an
            ...
            // Answering them needs, respectively, mobius's theme, its cell
            // metrics, and a clipboard-read policy.
            RioEvent::ColorRequest(..)
            | RioEvent::TextAreaSizeRequest(..)
            | RioEvent::ClipboardLoad(..) => {}

            // Features mobius does not implement yet, listed so that adding one
            ...
            // redraw bookkeeping mobius drives from Bevy instead. `RioEvent` is
...
/// DA1 capability parameters mobius must not advertise.
///
...
/// not exist and emit payloads mobius silently swallows.
const UNSUPPORTED_DA1_CAPABILITIES: &[&str] = &["4", "52"];

/// Rewrites an engine reply that would misreport mobius's capabilities or
/// identity, or returns `None` to send it through untouched.
///
/// rio-vt answers device-attribute and version queries on mobius's behalf, which
...
fn rewrite_reply(text: &str) -> Option<String> {
        ...
        .and_then(|rest| rest.strip_suffix('c'))
    {
        let kept = params
            ...
            .collect::<Vec<_>>();
        // Nothing to do when the engine already reported only what mobius
    ...
    if text.starts_with("\x1bP>|") && text.ends_with("\x1b\\") {
        return Some(format!("\x1bP>|mobius {}\x1b\\", env!("CARGO_PKG_VERSION")));
    }

    None
}

/// Encodes mobius's version the way the DA2 firmware field expects.
///
...
pub fn resolve_color(color: AnsiColor) -> CellColor {
    match color {
        ...
        AnsiColor::Named(named) => match named {
            // The terminal's own defaults. `Cursor` lands here because mobius
...
mod tests {
    ...
    /// rio-vt reports the engine's capabilities, not mobius's. Sixel and OSC 52
    ...
    #[test]
    fn secondary_device_attributes_report_mobiuss_version() {
        let mut harness = Harness::new(5, 20);
        harness.feed(b"\x1b[>0c");
    ...
    #[test]
    fn xtversion_reports_mobius_rather_than_rio() {
        let mut harness = Harness::new(5, 20);
        ...
        assert_eq!(
            reply,
            format!("\x1bP>|mobius {}\x1b\\", env!("CARGO_PKG_VERSION"))
        );
    ...
    /// rio-vt models `modifyOtherKeys` itself now, so mobius reads it from the
