...
impl<'a> TempleEditor<'a> {
    ...
    fn insert_object(&mut self, placement: ObjectPlacement) {
        ...
        self.objects.push(Some(PlacedGraphic {
            graphic: MobiusGraphic::new(
                MobiusGraphicSettings::new(placement.path)
                    .id(placement.id)
...
fn initial_lines() -> Vec<Vec<DocCell>> {
    [
        ...
        "",
        "Mobius is inspired by that direction. The goal here is not only to",
