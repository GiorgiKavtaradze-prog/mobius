...
/// Mobius graphic widget settings.
#[derive(Debug, Clone)]
pub struct MobiusGraphicSettings<'a> {
    /// Object identifier.
    pub id: u32,
    /// Asset path.
    pub path: Cow<'a, str>,
...
impl<'a> MobiusGraphicSettings<'a> {
    ...
    ///
    /// Normalization is enabled by default. With normalization enabled, Mobius
    ...
    /// for example a generated object that uses Mobius's scene coordinates or a
...
/// Mobius graphic widget.
pub struct MobiusGraphic<'a> {
    settings: MobiusGraphicSettings<'a>,
}

impl<'a> MobiusGraphic<'a> {
    /// Creates a graphic widget.
    pub fn new(settings: MobiusGraphicSettings<'a>) -> Self {
        Self { settings }
    }

    /// Returns the widget settings.
    pub fn settings(&self) -> &MobiusGraphicSettings<'a> {
        &self.settings
    }

    /// Returns mutable widget settings.
    pub fn settings_mut(&mut self) -> &mut MobiusGraphicSettings<'a> {
        &mut self.settings
    }
    ...
    pub fn register_sequence(&self) -> String {
        format!(
            "\x1b_mobius;g;r;id={};fmt={};path={};normalize={}\x1b\\",
            self.settings.id,
    ...
    pub fn register_payload_sequences_with_name(
        ...
        for (index, chunk_start) in (0..encoded.len()).step_by(PAYLOAD_CHUNK_SIZE).enumerate() {
            ...
            sequences.push(if index == 0 {
                format!(
                    "\x1b_mobius;g;r;id={};fmt={};source=payload;more={};name={};normalize={};{}\x1b\\",
                    self.settings.id,
                    self.settings.format.as_str(),
                    more,
                    name,
                ...
                format!(
                    "\x1b_mobius;g;r;id={};fmt={};source=payload;more={};{}\x1b\\",
                    self.settings.id,
                    self.settings.format.as_str(),
                    more,
                    chunk
        ...
        if sequences.is_empty() {
            sequences.push(format!(
                "\x1b_mobius;g;r;id={};fmt={};source=payload;more=0;name={};normalize={};\x1b\\",
                self.settings.id,
                self.settings.format.as_str(),
                name,
                u8::from(self.settings.normalize),
    ...
    pub fn place_sequence(&self, area: Rect) -> String {
        ...
        format!(
            "\x1b_mobius;g;p;id={};row={};col={};w={};h={};animate={};scale={};depth={};color={};brightness={};px={};py={};pz={};rx={};ry={};rz={};sx={};sy={};sz={}\x1b\\",
            self.settings.id,
            center_row,
            center_col,
            area.width.max(1),
    ...
    pub fn update_sequence(&self) -> String {
        format!(
            "\x1b_mobius;g;u;id={};animate={};scale={};depth={};color={};brightness={};px={};py={};pz={};rx={};ry={};rz={};sx={};sy={};sz={}\x1b\\",
            self.settings.id,
            u8::from(self.settings.animate),
            self.settings.scale,
            self.settings.depth,
    ...
    pub fn delete_sequence(&self) -> String {
        format!("\x1b_mobius;g;d;id={}\x1b\\", self.settings.id)
    }

    /// Returns the RGP sequence that deletes every Mobius graphic object.
    ///
    ...
    /// Deletes every Mobius graphic object.
    ///
    /// This writes the RGP delete-all sequence to stdout. It affects all RGP
    /// objects currently known to Mobius, not only objects created by this
    /// process.
    ///
    /// # Errors
    ///
...
impl Widget for &MobiusGraphic<'_> {
