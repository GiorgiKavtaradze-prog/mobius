...
Mobius Graphics Protocol (RGP) is a custom terminal protocol for inserting
...
but it is designed for Mobius's inline object layer and 3D renderer.
...
Mobius Graphics Protocol uses [APC] (Application Program Command):
...
- `mobius` is the protocol namespace
- `g` means graphics
...
Mobius treats protocol objects as inline terminal objects.
...
- `path`: object path known to Mobius
...
```text
ESC _ mobius;g;r;id=42;fmt=glb;source=payload;more=1;<chunk-1> ESC \
...
Delete all Mobius graphics objects:
...
Mobius Graphics Protocol is a terminal-native object protocol for 3D graphics.

Its key ideas are:

- register a renderable object
- place it in terminal cell space
- let Mobius render it as part of the terminal, including 3D mode

That is the foundation for "sprites on the command line" in Mobius, inspired by [TempleOS]-style
